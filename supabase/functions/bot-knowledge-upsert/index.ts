...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!content || typeof content !== "string") {
      return new Response(JSON.stringify({ ok: false, message: "Falta content." }), { status: 400 });
    }
    if (!LOVABLE_API_KEY) {
      return new Response(JSON.stringify({ ok: false, message: "LOVABLE_API_KEY no configurado en los secrets de la funcion." }), { status: 500 });
    }
    ...
    if (!embedRes.ok) {
      return new Response(JSON.stringify({ ok: false, message: `Error generando embedding (${embedRes.status}): ${await embedRes.text()}` }), { status: 502 });
    }
