...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!oportunidad_id || !text) {
      return new Response(JSON.stringify({ ok: false, message: "Falta oportunidad_id o text." }), { status: 400 });
    }
    ...
    if (!oportunidad?.telefono) {
      return new Response(JSON.stringify({ ok: false, message: "Esta oportunidad no tiene telefono de WhatsApp." }), { status: 400 });
    }
    ...
    if (!res.ok) {
      return new Response(JSON.stringify({ ok: false, message: `Evolution API error (${res.status}): ${await res.text()}` }), { status: 502 });
    }
