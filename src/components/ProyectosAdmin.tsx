...
import ProjectDailyKpis from './ProjectDailyKpis';
                      ...
                      <div key={k.id} className="bg-slate-50 border border-slate-200 p-3 rounded-lg flex flex-col justify-between">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs border-t border-slate-200 pt-2">
                          <div>
                            ...
                            <input value={k.actual} onChange={(event) => setKpis((current) => current.map((item) => item.id === k.id ? { ...item, actual: event.target.value } : item))} onBlur={(event) => updateKpiActual(k.id, event.target.value)} className="mt-1 w-full rounded border border-slate-200 bg-slate-100 px-2 py-1 font-bold text-blue-700 outline-none focus:border-blue-500" aria-label={`Valor actual de ${k.nombre}`} />
                          </div>
                          <div>
