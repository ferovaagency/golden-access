  ...
  return (
    <div className="space-y-8 animate-fade-in text-slate-900">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        {/* Form Container */}
        <form onSubmit={handleAddVenta} className="lg:col-span-4 bg-white border border-slate-200 pb-6 rounded-lg overflow-hidden space-y-4">
          ...
          <div className="px-5 space-y-4 text-xs font-sans">
            ...
            {/* Fecha */}
            <div>
              <label className="block text-slate-500 font-semibold mb-1 uppercase tracking-wider font-mono text-[10px]">Fecha de Venta</label>
              <input 
                type="date"
                value={fecha}
            ...
            {/* Cliente */}
            <div>
              <label className="block text-slate-500 font-semibold mb-1 uppercase tracking-wider font-mono text-[10px]">Cliente</label>
              <select 
                value={clienteId}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-slate-500 font-semibold mb-1 uppercase tracking-wider font-mono text-[10px]">Cantidad</label>
                <input 
                  type="number"
                  min="1"
                  value={cantidad}
              ...
              <div className="col-span-2">
                <label className="block text-slate-500 font-semibold mb-1 uppercase tracking-wider font-mono text-[10px]">P. Unitario ({moneda})</label>
                <input 
                  type="number"
                  min="0"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-slate-500 font-semibold mb-1 uppercase tracking-wider font-mono text-[10px]">Costo Unit. (COP)</label>
                <input 
                  type="number"
                  min="0"
                  value={costoUnitario}
              ...
              <div>
                <label className="block text-slate-500 font-semibold mb-1 uppercase tracking-wider font-mono text-[10px]">Moneda</label>
                <input 
                  type="text"
                  readOnly
                  value={moneda}
            ...
            <div className="space-y-4 p-4 bg-[#0f0e0c]/30 border border-slate-200/60 rounded-lg">
              ...
              {editingVentaId ? (
                ...
                <div className="space-y-4 text-xs">
                        ...
                        {activeAbonos.map((ab, index) => (
                          <div key={index} className="flex justify-between items-center p-2 bg-[#0f0e0c]/60 rounded border border-slate-200/50 text-[11px] font-mono">
                            <div>
                              ...
                              {ab.notas && <span className="block text-[10px] text-slate-400 pt-0.5">{ab.notas}</span>}
                            </div>
                            <button 
                              type="button" 
                  ...
                  <div className="p-3 bg-white/[0.01] border border-slate-200/75 rounded-lg space-y-3">
                    <span className="block font-mono text-[9px] uppercase tracking-widest text-blue-600 font-bold">
                      Registrar Abono en Otra Fecha:
                    </span>
                    
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-[8px] font-mono text-slate-400 uppercase mb-0.5">Monto de Abono</label>
                        <input 
                          type="number"
                          min="0"
                          placeholder="Monto..."
                      ...
                      <div>
                        <label className="block text-[8px] font-mono text-slate-400 uppercase mb-0.5">Fecha del Pago</label>
                        <input 
                          type="date"
                          value={newAbonoFecha}
                    ...
                    <div>
                      <label className="block text-[8px] font-mono text-slate-400 uppercase mb-0.5">Referencia / Comentarios</label>
                      <input 
                        type="text"
                ...
                <div className="space-y-2">
                  ...
                  {totalPactadoOriginal > 0 && (
                    <button
                      ...
                      className="w-full py-1 bg-white/[0.02] hover:bg-white/[0.05] text-[10px] font-mono text-blue-600 border border-slate-200 rounded"
                    >
                      Saldar Totalmente
                    </button>
                  )}
              ...
              {totalPactadoOriginal > 0 && (
                <div className="pt-2 border-t border-slate-200/30 space-y-1 text-[11px] font-mono">
                  ...
                  <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden mt-1">
                    <div 
                      className={`h-full rounded-full transition-all duration-300 ${adelanto >= totalPactadoOriginal ? 'bg-emerald-400' : 'bg-blue-600'}`}
            ...
            {/* LIVE PREVIEW BAR */}
            <div className="bg-white/[0.01] border border-slate-200 p-3 rounded-lg leading-relaxed space-y-1">
              <span className="text-[9px] font-mono tracking-wider uppercase text-slate-400 block text-center border-b border-slate-200/40 pb-1.5">
                Cómputo en Tiempo Real
              </span>
              <div className="flex flex-col gap-1 text-center font-mono text-[11px] pt-1 text-slate-900">
                <div>
                  <span className="text-slate-500">Total Pactado:</span>{' '}
                          ...
                          <td className="px-5 py-4">
                            <div className="font-mono text-slate-900">
                              {v.adelanto > 0 ? (
                                ...
                                <span className="text-slate-400 italic">Sin abonos</span>
                              )}
                            </div>
                            <div className="mt-1">
                              {actualEstado === 'Pagado' ? (
