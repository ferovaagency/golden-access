    ...
    <div className="ferova-v2-theme min-h-screen bg-[var(--ferova-canvas)] font-sans text-slate-900">
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/35 backdrop-blur-sm lg:hidden" onClick={onCloseMobileMenu}>
          ...
          >
            <div className="mb-4 flex items-center justify-between border-b border-[var(--ferova-line)] pb-4">
              <p className="font-display text-sm font-semibold text-slate-900">Menú</p>
              <button onClick={onCloseMobileMenu} className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--ferova-soft)] text-slate-500" aria-label="Cerrar menú">
                <X className="h-5 w-5" />
              </button>
            </div>
                  ...
                  <section key={section.id} className="rounded-[var(--ferova-radius-card)] border border-[var(--ferova-line)] p-2">
                    <button
                      ...
                      className={`flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-semibold font-display ${
                        isActive ? 'bg-[var(--ferova-brand)] text-white' : 'text-slate-500 hover:bg-[var(--ferova-soft)]'
                      }`}
                    >
                        ...
                        {section.items.map((item, index) => (
                          <Fragment key={item.id}>
                            {item.group && section.items.findIndex((candidate) => candidate.group === item.group) === index && (
                              <p className="px-3 pb-1 pt-3 text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400 first:pt-1">{item.group}</p>
                            )}
                            <button
                              onClick={() => onNavigateTab(item.id)}
                              className={`min-h-11 w-full rounded-xl px-3 py-2 text-left ${
                                activeTab === item.id ? 'bg-[var(--ferova-ai)] text-[var(--ferova-navy)]' : 'text-slate-500 hover:bg-[var(--ferova-soft)]'
                              }`}
                            >
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 hidden flex-col border-r border-[var(--ferova-line)] bg-[var(--ferova-surface)] transition-[width] duration-200 lg:flex ${sidebarCollapsed ? 'w-[72px]' : 'w-60'}`}>
        ...
        <div className={`flex-1 overflow-y-auto ${sidebarCollapsed ? 'px-2 py-4' : 'px-3 py-4'}`}>
          {!sidebarCollapsed && <p className="mb-2 px-3 text-[9px] font-bold uppercase tracking-[.16em] text-slate-400">Navegación</p>}
          <PrimaryNavigation
            sections={sections}
        ...
        <div className={`border-t border-[var(--ferova-line)] ${sidebarCollapsed ? 'p-2' : 'p-3'}`}>
          <button type="button" className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-xs font-medium text-slate-500 hover:bg-[var(--ferova-soft)] ${sidebarCollapsed ? 'justify-center' : ''}`} title={sidebarCollapsed ? 'Ayuda' : undefined}><HelpCircle className="h-4 w-4" />{!sidebarCollapsed && ' Ayuda'}</button>
          <button type="button" onClick={() => setSidebarCollapsed((value) => !value)} className={`mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-xs font-medium text-slate-500 hover:bg-[var(--ferova-soft)] ${sidebarCollapsed ? 'justify-center' : ''}`} title={sidebarCollapsed ? 'Expandir navegación' : 'Contraer navegación'}>
        ...
        <div className="flex w-full min-w-0 flex-1 gap-4 px-3 pb-5 sm:px-5">
          <main className="min-w-0 flex-1">
            <div className="mx-auto mb-4 max-w-[1380px] border-b border-[var(--ferova-line)] pb-3 pt-4">
              <div className="mb-3 flex items-center gap-2 text-xs text-slate-500" aria-label="Ubicación actual">
