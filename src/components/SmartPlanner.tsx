...
import { Sparkles, Wand2, Loader2, Check, Play, Pause, Clock, Trash2, ChevronLeft, ChevronRight, Sunrise, AlertTriangle, Lightbulb, TrendingUp, Info, Lock, Edit2, X, CalendarDays, Columns3, List, LayoutGrid, SlidersHorizontal, Network } from 'lucide-react';
import { usePlanner } from '../hooks/usePlanner';
import { plannerService, type PlannerBlock, type PlannerCategory, type PlannerDraft, type PlannerEnergy, type PlannerTask, type TareaDeEmpresa, type AccountPerson } from '../lib/plannerService';
...
export default function SmartPlanner() {
  ...
  // Personas con acceso a esta cuenta (para asignar) y si hay más de una
  // empresa alcanzable (para ofrecer la vista consolidada).
  const [personas, setPersonas] = useState<AccountPerson[]>([]);
  const [hayVariasEmpresas, setHayVariasEmpresas] = useState(false);
  useEffect(() => {
    let vivo = true;
    plannerService.listAccountPeople().then((gente) => { if (vivo) setPersonas(gente); }).catch(() => {});
    plannerService.listTasksAllCompanies()
      .then((tareas) => { if (vivo) setHayVariasEmpresas(new Set(tareas.map((t) => t.account_id)).size > 1); })
      .catch(() => {});
    return () => { vivo = false; };
  }, []);
  ...
  const saveTaskEditor = async (event: React.FormEvent) => {
    ...
    if (taskRepeatDays.length && (!taskDate || !taskTime)) {
      setTaskSaveNotice('Para repetir una tarea, elige la fecha inicial y una hora. Así se reservan sus ocurrencias en la agenda.');
      return;
    }
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-1 rounded-xl bg-slate-100 p-1" aria-label="Vista del planner">
          {([
            ...
            // Sólo tiene sentido con más de una empresa alcanzable: en una
            // cuenta suelta sería una pestaña que repite lo mismo.
            ...(hayVariasEmpresas ? [['empresas', 'Todas las empresas', Network] as const] : []),
      ...
      {plannerView === 'empresas' && <PlannerTodasLasEmpresas />}
      ...
      <section className="lg:order-1">
        ...
        {editingTask && (
          ...
          <form onSubmit={saveTaskEditor} className="my-6 w-full max-w-3xl rounded-2xl border border-blue-200 bg-white p-4 shadow-2xl">
            <div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-slate-900">{editingTask.title}</p><p className="text-[11px] text-slate-600">
              Ábrela para interactuar: inicia/finaliza, dale contexto a la IA y ajusta lo que necesites.
              {/* Quién la creó: con varias personas en la empresa, saberlo evita
                  tener que preguntar por chat de dónde salió una tarea. */}
              {editingTask.created_by && personas.length > 1 && (
                <span className="ml-1 text-slate-500">· Creada por {personas.find((persona) => persona.user_id === editingTask.created_by)?.email ?? 'otra persona'}</span>
              )}
            </p></div><button type="button" onClick={() => setEditingTask(null)} className="text-slate-500 hover:text-slate-900" aria-label="Cerrar"><X className="h-4 w-4" /></button></div>
            {/* Acciones rápidas */}
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              {/* Responsable: con varias personas en la misma empresa, una tarea
                  sin dueño no es de nadie. Sólo aparece si hay a quién asignar. */}
              {personas.length > 1 && (
                <label className="inline-flex items-center gap-1.5 text-xs text-slate-600">Responsable
                  <select
                    value={editingTask.responsable_user_id ?? ''}
                    onChange={async (event) => {
                      const valor = event.target.value || null;
                      setEditingTask({ ...editingTask, responsable_user_id: valor });
                      await p.setResponsable(editingTask.id, valor);
                    }}
                  ...
                  >
                    ...
                    {personas.map((persona) => <option key={persona.user_id} value={persona.user_id}>{persona.email}</option>)}
                  </select>
                </label>
              )}
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <fieldset className="sm:col-span-2 lg:col-span-4 rounded-lg border border-blue-200 bg-white p-3"><legend className="px-1 text-xs text-slate-600">Repetir semanalmente</legend><p className="mb-2 text-[10px] text-slate-500">Elige los días. Se crearán bloques protegidos desde “Programar desde” hasta la fecha final (o durante los próximos 90 días).</p><div className="flex flex-wrap gap-2">{[['D',0],['L',1],['M',2],['X',3],['J',4],['V',5],['S',6]].map(([label, day]) => <button type="button" key={String(day)} onClick={() => setTaskRepeatDays((days) => days.includes(Number(day)) ? days.filter((value) => value !== Number(day)) : [...days, Number(day)].sort())} className={`h-8 w-8 rounded-full text-xs font-semibold ${taskRepeatDays.includes(Number(day)) ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600'}`}>{label}</button>)}<label className="ml-2 text-xs text-slate-600">Hasta<input type="date" disabled={!taskRepeatDays.length} value={taskRepeatUntil} onChange={(event) => setTaskRepeatUntil(event.target.value)} className="ml-2 rounded border border-blue-200 px-2 py-1" /></label><label className="ml-auto flex items-center gap-2 text-xs text-slate-700"><input type="checkbox" checked={taskSyncGoogle} disabled={!taskDate || !taskTime} onChange={(event) => setTaskSyncGoogle(event.target.checked)} /> Sincronizar con Google Calendar</label></div></fieldset>
...
/**
 * Las tareas abiertas de todas las empresas alcanzables, agrupadas por empresa.
 *
 * Los datos vienen de una función de base (`planner_tasks_todas_las_empresas`)
 * y no de una consulta normal: desde el navegador la RLS devuelve sólo la
 * cuenta activa, a propósito, porque el resto de la aplicación asume que lo que
 * llega es "lo mío". Ésta es la excepción explícita — y por eso cada tarea
 * viene con el nombre de su empresa, para que no se vean mezcladas sin saber
 * de quién son.
 */
function PlannerTodasLasEmpresas() {
  const [tareas, setTareas] = useState<TareaDeEmpresa[] | null>(null);
  useEffect(() => { plannerService.listTasksAllCompanies().then(setTareas).catch(() => setTareas([])); }, []);

  if (tareas === null) {
    return <div className="flex items-center gap-2 rounded-2xl border border-[var(--line)] bg-white p-5 text-xs text-slate-500"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Cargando las tareas de todas las empresas…</div>;
  }
  if (!tareas.length) {
    return <div className="rounded-2xl border border-dashed border-[var(--line)] bg-white p-6 text-center text-sm text-slate-500">No hay tareas abiertas en ninguna empresa.</div>;
  }

  const porEmpresa = new Map<string, TareaDeEmpresa[]>();
  for (const t of tareas) porEmpresa.set(t.empresa, [...(porEmpresa.get(t.empresa) ?? []), t]);
  const hoy = new Date().toISOString().slice(0, 10);

  return (
    <section className="space-y-4">
      <p className="text-[11px] text-slate-500">
        Sólo lectura: para trabajar en una tarea, cambia a esa empresa en el selector de la cabecera.
      </p>
      {[...porEmpresa.entries()].map(([empresa, lista]) => (
        <div key={empresa} className="overflow-hidden rounded-2xl border border-[var(--line)] bg-white">
          <div className="flex items-center gap-2 border-b border-[var(--line)] bg-slate-50 px-4 py-2.5">
            <Network className="h-3.5 w-3.5 text-slate-500" />
            <span className="text-xs font-semibold text-slate-900">{empresa}</span>
            <span className="ml-auto rounded-full bg-white px-2 py-0.5 text-[10px] font-semibold text-slate-500 ring-1 ring-slate-200">
              {lista.length} abierta{lista.length === 1 ? '' : 's'}
            </span>
          </div>
          <ul className="divide-y divide-slate-100">
            {lista.map((t) => {
              const vencida = t.deadline && t.deadline.slice(0, 10) < hoy;
              return (
                <li key={t.id} className="flex flex-wrap items-center gap-2 px-4 py-2.5 text-xs">
                  <span className="min-w-0 flex-1 truncate text-slate-800">{t.title}</span>
                  {t.responsable_email && (
                    <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-medium text-blue-700">{t.responsable_email}</span>
                  )}
                  <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-600">{STATUS_LABEL[t.status] || t.status}</span>
                  {t.deadline && (
                    <span className={`font-mono text-[10px] ${vencida ? 'font-semibold text-red-600' : 'text-slate-500'}`}>
                      {vencida ? 'venció ' : 'vence '}{t.deadline.slice(0, 10)}
                    </span>
                  )}
                </li>
              );
