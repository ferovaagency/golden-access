...
function ReportsOverview({ reports }: { reports: CeoReport[] }) {
  ...
  // --- Análisis legible por gráfica (en palabras, no solo barras) ---
  const pct = (v: number) => `${v >= 0 ? '+' : ''}${Math.round(v)}%`;
  const ingLast = history.length ? history[history.length - 1].ingresos : 0;
  const ingPrev = history.length > 1 ? history[history.length - 2].ingresos : 0;
  const ingDelta = ingPrev > 0 ? ((ingLast - ingPrev) / ingPrev) * 100 : 0;
  const utilLast = history.length ? history[history.length - 1].utilidad : 0;
  const margenReal = ingLast > 0 ? (utilLast / ingLast) * 100 : 0;
  const promedioIng = history.length ? history.reduce((s, h) => s + h.ingresos, 0) / history.length : 0;
  const marginPctNorm = Math.round(margin * (Math.abs(margin) <= 1 ? 100 : 1));

  const analisisEvolucion = history.length < 2
    ? 'Aún no hay suficientes períodos para ver una tendencia. Genera más reportes para que esta línea cuente una historia.'
    : `Tus ingresos del último período fueron ${formatCop(ingLast)}, ${ingDelta >= 0 ? 'arriba' : 'abajo'} ${pct(Math.abs(ingDelta))} frente al período anterior (${formatCop(ingPrev)}). El promedio de la ventana es ${formatCop(promedioIng)}${ingLast >= promedioIng ? ' — vas por encima de tu promedio.' : ' — vas por debajo de tu promedio, ojo.'}`;
  const analisisComparativo = ingLast <= 0
    ? 'Sin ingresos en el último período para comparar contra la utilidad.'
    : `En el último período, de cada $100 que facturaste te quedaron ${formatCop(Math.round(margenReal))}${''} de utilidad (margen ${Math.round(margenReal)}%). ${margenReal >= 30 ? 'Margen sano.' : margenReal >= 15 ? 'Margen ajustado: revisa costos o precios.' : 'Margen bajo: aquí es donde más se te escapa la plata.'}`;
  const analisisConjunto = `Salud del negocio en ${health}%. ${ingDelta >= 0 ? 'Los ingresos crecen' : 'Los ingresos caen'} (${pct(ingDelta)}) con un margen del ${marginPctNorm}%. ${health >= 70 && margenReal >= 20 ? 'Vas bien: sostén el ritmo y protege el margen.' : health < 50 ? 'Prioridad: estabilizar. Enfócate en cobrar lo pendiente y recortar el gasto que no rinde.' : 'Zona intermedia: sube ingresos sin soltar el margen; ataca primero el 20% que da el 80%.'}`;
  ...
  return <div className="space-y-4">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <article className="flex items-center justify-between rounded-[var(--ferova-radius-card)] border border-[var(--ferova-line)] bg-white p-4 shadow-[var(--ferova-shadow)]"><div><p className="text-[9px] font-bold uppercase tracking-[.15em] text-slate-400">Health del negocio</p><p className="mt-2 text-xl font-semibold text-slate-950">{health}%</p><p className="mt-1 text-[11px] text-slate-500">Ecosistema de indicadores</p></div><div className="grid h-16 w-16 place-items-center rounded-full" style={{ background: `conic-gradient(#2563EB ${health * 3.6}deg, #ececee 0)` }}><div className="grid h-11 w-11 place-items-center rounded-full bg-white text-xs font-bold text-[#2563EB]">{health}</div></div></article>
    </div>
    <div className="grid gap-4 xl:grid-cols-[1.35fr_.85fr]">
      <article className="rounded-[var(--ferova-radius-card)] border border-[var(--ferova-line)] bg-white p-4 shadow-[var(--ferova-shadow)]"><div className="mb-3"><p className="text-[9px] font-bold uppercase tracking-[.15em] text-slate-400">Evolución de ingresos</p><h2 className="mt-1 text-sm font-semibold text-slate-900">¿Estás creciendo o cayendo?</h2></div><div className="h-56"><ResponsiveContainer width="100%" height="100%"><AreaChart data={history}><defs><linearGradient id="reportRevenue" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2563EB" stopOpacity={.28}/><stop offset="100%" stopColor="#2563EB" stopOpacity={0}/></linearGradient></defs><CartesianGrid vertical={false} stroke="#ececee"/><XAxis dataKey="period" axisLine={false} tickLine={false} tick={{fontSize:10,fill:'#8b8b91'}}/><YAxis hide/><Tooltip formatter={(value) => formatCop(Number(value) || 0)} contentStyle={{borderRadius:10,border:'1px solid #e4e5e7',fontSize:11}}/><Area type="monotone" dataKey="ingresos" stroke="#2563EB" strokeWidth={2.5} fill="url(#reportRevenue)" animationDuration={900}/></AreaChart></ResponsiveContainer></div><p className="mt-3 border-t border-slate-100 pt-3 text-xs leading-relaxed text-slate-600"><b className="text-slate-900">Qué dice:</b> {analisisEvolucion}</p></article>
      <article className="rounded-[var(--ferova-radius-card)] border border-[var(--ferova-line)] bg-white p-4 shadow-[var(--ferova-shadow)]"><div className="mb-3 flex items-start justify-between gap-2"><div><p className="text-[9px] font-bold uppercase tracking-[.15em] text-slate-400">Ingresos vs utilidad</p><h2 className="mt-1 text-sm font-semibold text-slate-900">¿Cuánto te queda de lo que facturas?</h2></div><div className="flex flex-col gap-1 text-[10px] text-slate-500"><span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full" style={{background:'#2563EB'}}/>Ingresos</span><span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full" style={{background:'#16a34a'}}/>Utilidad</span></div></div><div className="h-56"><ResponsiveContainer width="100%" height="100%"><BarChart data={history.slice(-5)} layout="vertical"><CartesianGrid horizontal={false} stroke="#ececee"/><XAxis type="number" hide/><YAxis dataKey="period" type="category" axisLine={false} tickLine={false} width={42} tick={{fontSize:10,fill:'#8b8b91'}}/><Tooltip formatter={(value) => formatCop(Number(value) || 0)} contentStyle={{borderRadius:10,border:'1px solid #e4e5e7',fontSize:11}}/><Bar dataKey="ingresos" fill="#2563EB" radius={[0,6,6,0]} barSize={9}/><Bar dataKey="utilidad" fill="#16a34a" radius={[0,6,6,0]} barSize={9}/></BarChart></ResponsiveContainer></div><p className="mt-3 border-t border-slate-100 pt-3 text-xs leading-relaxed text-slate-600"><b className="text-slate-900">Qué dice:</b> {analisisComparativo}</p></article>
    </div>
    {/* Análisis del conjunto */}
    <article className="rounded-[var(--ferova-radius-card)] border border-blue-200 bg-blue-50/50 p-4">
      <p className="text-[9px] font-bold uppercase tracking-[.15em] text-blue-700">Análisis del conjunto</p>
      <p className="mt-2 text-sm leading-relaxed text-slate-800">{analisisConjunto}</p>
    </article>
  </div>;
}
