...
- [x] Fase 4 — IA + Planner (reskin por CSS, ver nota de técnica abajo).
- [x] Fase 5 — Finanzas + CRM (mismo mecanismo).
- [x] Fase 6 — Landing + demos (sin la escena 3D, ver Fase 7). `src/routes/LandingV2.tsx`, ruta nueva y aditiva en `/landing-v2` (no reemplaza `/` ni `/landing` — se activa cuando se valide Core Web Vitals, por eso convive con la actual). Sin llamadas a backend. Las 10 secciones del manual: Hero (con preview 3D-ligero tipo tilt-card, Nivel A), Prueba inmediata, Problema (fragmentos que convergen, animado), Demo Executive Control Center (interactiva, fixture, marcada "Datos de demostración"), Módulos, 4 mini-demos (Planner/Finanzas/CRM/IA, reveal por scroll con `whileInView`), Cómo funciona (5 pasos), Casos de uso (tabs Freelancer/Consultor/Agencia/Small business, sin recargar), Planes (con nota Founder Access) y FAQ. Verificado en el navegador: las 12 regiones renderizan, el selector de período y los tabs de casos de uso funcionan (probado con clicks reales), sin errores de consola.
- [x] Fase 7 — 3D + optimización. Construida en la rama separada `feat/ferova-ui-v2-3d` (el manual lo pide explícitamente en rama aparte, no mezclada con `main`) — `src/components/three/{ProductUniverse,HeroUniverse}.tsx`, dependencias `three@0.185`/`@react-three/fiber@9`/`@react-three/drei@10`. Escena: núcleo/orb dorado + 4 tarjetas orbitando (Finanzas/CRM/Planner/Reportes), inclinación por cursor, auto-rotación, `Sparkles` de acento, degradación si el FPS sostenido cae bajo 45. `HeroUniverse` decide en `useEffect` (nunca bloquea el LCP): `React.lazy`/`Suspense` + poster CSS (el mismo `TiltPreviewCard` de Nivel A) mientras carga, en `<768px` o con `prefers-reduced-motion`; pausa el canvas vía Page Visibility API cuando la pestaña no está visible. **Verificado en navegador, no solo por build**: a 1280px monta exactamente 1 `<canvas>` con contexto WebGL2 activo; a 375px monta 0 canvas (poster puro). `dpr` limitado a `[1, 1.5]`. El chunk `ProductUniverse` pesa ~288 KB gzip — lazy, solo se descarga en desktop sin reduced-motion, nunca en `main` ni en el resto de la app. **Alcance no cubierto en esta pasada** (documentado, no fingido): el manual también pide que el scroll separe las capas y abra una tarjeta para mostrar un demo — esta iteración solo tiene la inclinación por cursor y la órbita ambiental, no la coreografía scroll-linked completa.

### Nota de técnica: Fase 4/5 por CSS, no por componente

A diferencia de las Fases 2/3/6 (componentes nuevos con `isFerovaUiV2Enabled()`), Planner, Finanzas y CRM son módulos grandes y con interacción real (drag-and-drop, Calendar sync, formularios, kanban) — reescribirlos componente a componente en una sola pasada era el riesgo exacto que el premortem del manual advierte ("El rediseño rompe permisos o navegación"). En su lugar se reusó el mecanismo que el propio repo ya tenía para esto (`.ferova-light-theme` en `src/index.css`, que ya reskinea el Paywall por CSS sin tocar su JSX): una clase nueva `.ferova-v2-theme` que sobreescribe por CSS las clases Tailwind ya usadas en esos módulos (`bg-slate-50`→canvas, `text-blue-600`→borgoña, `bg-blue-50`+`text-blue-700`→lavanda+gold para insignias de IA, etc.), aplicada por `App.tsx` alrededor de `mainContent`, `periodBarNode` y `aiSidebarNode` cuando el flag está encendido. Cero cambios de lógica/JSX en `SmartPlanner.tsx`, `FinanceOperativa.tsx`, `CustomerCRM.tsx`, `AISidebar.tsx` ni ningún otro módulo — drag-and-drop, streaming del chat, Calendar sync y cálculos financieros quedan intactos. Es un reskin real pero no pixel-perfect (no cubre cada clase de color existente); refinarlo módulo por módulo sigue siendo trabajo futuro legítimo si se quiere el nivel de detalle de la Fase 3.
