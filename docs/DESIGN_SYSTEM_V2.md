...
- [x] Fase 7 — 3D + optimización. Construida en la rama separada `feat/ferova-ui-v2-3d` (el manual lo pide explícitamente en rama aparte, no mezclada con `main`) — ver esa rama para `ProductUniverse.tsx`, dependencias `three`/`@react-three/fiber`/`@react-three/drei` y el hero de `LandingV2` con `React.lazy`/`Suspense` + poster estático + desactivado en <768px.
