...
`src/lib/auditLogService.ts` expone `logSuggestion`, `resolveSuggestion`, `listPendingSuggestions`, `countTodayAutoActions`, `logCalculationRun`. **Primer consumidor real**: la reprogramación automática de tareas vencidas del Planner. Existen dos mecanismos complementarios (no duplicados: el que corre primero mueve la tarea y el otro ya no encuentra nada que mover): el cron diario `roll_forward_missed_planner_tasks()` (5:10, respeta días laborales de `business_profile`) y el barrido client-side `rescheduleOverdueTasks()` al abrir el Planner. **Ambos** registran cada movimiento en `audit_log` con `actor:'system'` y `action:'reprogramado_automatico'` (el cron lo hace desde la migración `20260723020000`), y el banner del Planner cuenta desde `audit_log` (`countTodayAutoActions`) para avisar al usuario sin importar cuál mecanismo movió la tarea.
...
Arquitectura de 4 capas (raw/canonical/metrics/insights), separación completa de ingresos contratados/causados/cobrados con probabilidad de cobro ponderada aplicada al flujo de caja real (`cashflowService.ts` sigue binario), centralización completa del embudo de CRM, sistema de memoria/aprendizaje de la IA sobre `audit_log`/`user_corrections`, motor de reconciliación de transacciones y rediseño del onboarding con tarifa mínima sugerida — son cada uno un proyecto real de varios días.

**Priority Score del Planner (4.13)**: cada tarea incorpora cuatro entradas explícitas de 1 a 5: impacto financiero, impacto cliente, riesgo al aplazar y facilidad de ejecución. La clasificación por Brain Dump puede sugerirlas solo con evidencia del texto y deja `3` cuando no hay suficiente contexto; el usuario puede modificarlas en el editor. `planner-plan-day` ordena la agenda con la fórmula del manual: `0,30 urgencia + 0,25 financiero + 0,20 cliente + 0,15 riesgo + 0,10 facilidad`. El resultado aparece como `Score` y explica la ponderación; conserva la prioridad manual, fecha límite y tareas postergadas como desempates. También permite declarar dependencias: una tarea dependiente no entra al plan automático hasta que estén completadas sus predecesoras. Los valores no se presentan como una verdad financiera: son señales editables y auditables en el registro de la tarea.
...
**Cobros manuales por archivo (decisión de producto)**: no se agregan conectores de Wompi, Mercado Pago ni Stripe. En `Ventas`, el botón **Importar cobros CSV** acepta archivos separados por coma o punto y coma e interpreta encabezados habituales en español o inglés (`fecha/date`, `monto/valor/amount`, `cliente/customer`, `servicio/producto`, `referencia/transaction id`, `moneda/currency`). Presenta cuántas filas puede asociar de manera única y solo entonces agrega los abonos: nunca sustituye ventas ni abonos existentes. La referencia externa (o la fila, si no existe referencia) queda marcada en la nota para que repetir el mismo archivo no duplique el cobro. Las filas ambiguas o sin cliente quedan pendientes de revisión, en lugar de atribuir dinero al cliente equivocado. Es una implementación controlada de la importación/mapeo/vista previa/confirmación de 4.14; no pretende todavía reemplazar la arquitectura raw/canonical de 4.15.

**Planner — horario y calendario protegido**: `planner-plan-day` normaliza los timestamps ISO de Google Calendar y de los bloques protegidos a minutos de `America/Bogota` antes de buscar espacios disponibles. Así un evento a las 09:00 de Colombia no se interpreta como uno a las 14:00 por la ejecución UTC de la Edge Function. El plan conserva el horario laboral configurado y esos eventos como ocupados; este ajuste corrige la condición necesaria para cumplir la regla de 4.13 de no desplazar compromisos importantes sin aprobación.
